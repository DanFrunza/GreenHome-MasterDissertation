"""
Backtesting — SMALL VOLUME regime (< 336 training points → Seasonal Naive selected).

Cutoff: 2026-06-18.  Uses last 100 rows before cutoff → ~99 training pts (Naive regime).
The production system would choose Seasonal Naive here.
Both models are run to compare their MAE in this data-scarce regime.
...
def _ridge_no_lag(times_hist, values_hist, times_future):
    from sklearn.linear_model import Ridge
    from sklearn.preprocessing import StandardScaler
    X_train, y_train = [], []
    for t, v in zip(times_hist, values_hist):
        if v is None:
            continue
        X_train.append(_calendar_features(t))
        y_train.append(v)
    ...
    model = Ridge(alpha=1.0)
    model.fit(X_train_s, y_train)
    X_pred = np.array([_calendar_features(t) for t in times_future], dtype=float)
    return [max(0.0, p) for p in model.predict(scaler.transform(X_pred)).tolist()]

def _random_forest_no_lag(times_hist, values_hist, times_future):
...
def backtest_entity(conn, home_id, entity_id, cutoff_date, test_hours=168,
    ...
    t_train, v_train = load_hourly_aggregations(
        cur, home_id, entity_id, unit, device_class, None, cutoff_date,
    ...
    ridge_preds = _ridge_no_lag(t_train, v_train, t_test)
    rf_preds    = _random_forest_no_lag(t_train, v_train, t_test)
...
# last 100 rows → ~99 training pts → Seasonal Naive selected in production
...
OUTPUT_DIR = '/home/dan/Documents/Dizertatie/New/Demo/test/small_volume'
...
if __name__ == '__main__':
    ...
    if all_results:
        plot_combined(all_results, f'{OUTPUT_DIR}/combined_backtest_small.png')
